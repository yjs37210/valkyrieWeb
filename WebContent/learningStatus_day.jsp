			...
			<div class="container-cal">
				...
				<div class="study-time" border='1'>
					<div class="study-time-e">
						<p>총 공부시간</p>
						<input type="text" id="dTime" value="${vo.dTime}" readonly>
					</div>
					<div class="study-time-e">
						<p>시작시간</p>
						<c:set var="studyStart" value="${vo.studyStart}" />
						<input type="text" id="studyStart"
							value="${fn:substring(studyStart,11,19)}" readonly>
					</div>
					<div class="study-time-e">
						<p>종료시간</p>
						<c:set var="studyEnd" value="${vo.studyEnd}" />
						<input type="text" id="studyEnd"
							value="${fn:substring(studyEnd,11,19)}" readonly>
					</div>
				</div>
				<!-- // .my-calendar -->
			</div>
							...
							<div class="col-lg-9 col-md-9">
								...
								<div class="chart-textdiv">
									<div class="chart-ratio">
										<h3>
											<p id="realStudyDTime" class="excert e1">공부 시간 : ${vo.dTime}</p>
										</h3>
									</div>
									<div class="chart-ratio">
										<h3>
											<p id="realRestDTime" class="excert e2">쉬는 시간 : ${vo.restDTime}</p>
										</h3>
									</div>
								</div>
							</div>
			...
			if (e.target.classList.contains('day')) {
				...
				$(function(){
					$.ajax({
						...
						url:'dSTimeStatistics.do?yy='+yy+'&mm='+mm+'&day='+day,
						// 안되면utf-8로 바꾸기
						dataType:'json',
						success:function(result){
							console.log(result);
							$('#dTime').val(result.dTime);
							$('#studyStart').val(result.studyStart);
							$('#studyEnd').val(result.studyEnd);
							
							$('#realStudyDTime').html('공부 시간 : '+result.dTime);
							$('#realRestDTime').html('쉬는 시간 : '+result.restDTime);
							chart = new Chart(ctx, {
								// The type of chart we want to create
								type: 'pie',
								...
								data: {
									...
									datasets: [{
										...
										borderColor: '#fff',
										// 퍼센테이지로 뽑아야할 듯.
										data: [result.dSec, result.restDSec]
									}]
								},
								// Configuration options go here
								options: {}
						...
						error:function(){
							$('#dTime').val('00:00:00');
							$('#studyStart').val('00:00:00');
							$('#studyEnd').val('00:00:00');
							$('#realStudyDTime').html('공부 시간 : 00:00:00');
							$('#realRestDTime').html('쉬는 시간 : 00:00:00');
							chart = new Chart(ctx, {
								// The type of chart we want to create
								type: 'pie',
